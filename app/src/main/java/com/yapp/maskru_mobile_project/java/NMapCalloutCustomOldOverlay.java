 ...
 */
package com.yapp.maskru_mobile_project.java;
...
public class NMapCalloutCustomOldOverlay extends NMapCalloutOverlay {
	...
	private void adjustTextBounds(NMapView mapView) {
		...
		if (mTitleTruncated == null || mWidthTitleTruncated != mapViewWidth) {
			...
			if (DEBUG) {
				Log.i("NMAP", "adjustTextBounds: maxWidth=" + maxWidth + ", mMarginX=" + mMarginX + ", mPaddingX="
					+ mPaddingX);
			}
			...
			if (DEBUG) {
				Log.i("NMAP", "adjustTextBounds: mTitleTruncated=" + mTitleTruncated + ", length="
		...
		if (DEBUG) {
			Log.i("NMAP", "adjustTextBounds: mTempRect.width=" + mTempRect.width() + ", mTempRect.height="
				+ mTempRect.height());
		}
